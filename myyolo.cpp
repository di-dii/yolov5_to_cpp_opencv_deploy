...
YOLO::YOLO(Net_config config)
{
	...
	modelFile += ".onnx";
	this->net = readNetFromONNX(modelFile); //readNet
}
...
void YOLO::detect(Mat& frame)
{
	...
	for (n = 0; n < 3; n++)   ///尺度
	{
		...
		for (q = 0; q < 3; q++)    ///anchor数
		{
			...
			///float* pdata = (float*)outs[n].data + q * nout * area;  //指向第n个尺度的输出
			for (i = 0; i < num_grid_y; i++)
			{
				for (j = 0; j < num_grid_x; j++)
				{
                    float* pdata=(float*)outs[n].data+row_ind*nout;
					//float box_score =  pdata[85*(j+i*num_grid_y)+4];  //pdata[4 * area + i * num_grid_x + j];
					...
					if (box_score > this->objThreshold)
					{
						...
						{
							class_socre = pdata[5+c];//pdata[(c + 5) * area + i * num_grid_x + j];
						...
						{
							float cx = (pdata[0] * 2.f - 0.5f + j) * this->stride[n];  ///cx    ///pdata[i * num_grid_x + j]
							float cy = (pdata[1] * 2.f - 0.5f + i) * this->stride[n];   ///cy     ///pdata[area + i * num_grid_x + j]
							float w = powf(pdata[2] * 2.f, 2.f) * anchor_w;   ///w                       ///pdata[2 * area + i * num_grid_x + j]
							float h = powf(pdata[3]* 2.f, 2.f) * anchor_h;  ///h                       ///pdata[3 * area + i * num_grid_x + j]
